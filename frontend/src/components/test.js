...
export class Test {
    constructor() {
        ...
        showQuestion() {
            ...
            localStorage.setItem('answerId', JSON.stringify(this.quiz));
        }
        chooseAnswer() {
        ...
        move(action) {
            ...
            if (existingResult) {
                ...
                localStorage.setItem('resultId', JSON.stringify(this.userResult));
            }
      ...
      async  complete() {
                ...
                try {
                    const result = await CustomHttp.request(config.host + '/tests/' + this.routeParams.id + '/pass', "POST", {
                        userId: userInfo.userId, results: this.userResult
                    })
                    if (result) {
                        if (result.error) {
                            throw new Error(result.error);
